...
const SelectInput = (props: ISelectInputProps) => {
  ...
  const selectName = options?.find(f => f.value === value)
  useClickOutClose({ onClickOut: () => setOpenMenu(false), isOpen: openMenu })

  const filteredList =
    searchForm.length > 0
      ? options?.filter(f => f.name.toLowerCase().indexOf(searchForm.toLowerCase()) >= 0)
      : options
          ...
          >
            ...
            >
              {(!options || filteredList?.length === 0) && (
                <div className="py-[15px] px-[20px] cursor-pointer text-[13px] text-center">
                  Ничего не найдено
                </div>
              )}

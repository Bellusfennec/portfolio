...
const Table = (props: ITableProps) => {
  ...
  const classButtonPage =
    'flex justify-center py-[10px] px-[11px] text-[14px] font-medium min-w-[40px]'

  return (
    <div
      className={
        'border border-grey-#5 border-b-grey-#5 rounded-[8px] overflow-hidden bg-white' +
        (className ? ' ' + className : '')
      }
    >
      ...
      <div className="flex flex-col divide-y divide-grey-#5">
        {sortedData?.length === 0 && (
          <div className="flex justify-center w-full p-[24px]">Ничего не найдено</div>
        )}
      ...
      {true && (
        <div className="flex justify-between py-[14px] px-[24px] border-t border-grey-#5">
          <div className="w-full flex justify-start">
            {currentPage !== 1 && (
              <Button onClick={() => setCurrentPage(state => state - 1)}>
                ...
                <span className="font-medium text-[14px]">Назад</span>
              </Button>
            )}
          </div>
          <div className="w-full flex justify-center">
            ...
            <p className={classButtonPage + ' bg-purple-#2 text-purple-#1 rounded-[8px]'}>
              {currentPage}
            </p>
          ...
          <div className="w-full flex justify-end">
            {currentPage !== countPages && (
              <Button onClick={() => setCurrentPage(state => state + 1)}>
                <span className="font-medium text-[14px]">Вперед</span>

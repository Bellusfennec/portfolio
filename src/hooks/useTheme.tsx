...
interface IContextTheme {
  isDark: boolean
  handlerChange?: (props: IHandlerChange) => void
}
...
const localTheme = !isServer && localStorage.getItem(DARK) === 'dark'

const ThemeContext = React.createContext<IContextTheme | undefined | boolean>({
  isDark: systemTheme,
})
...
export const ThemeProvider = (props: IThemeProviderProps) => {
  ...
  const handlerChange = ({ value }: IHandlerChange) => setDark(value)

  return <ThemeContext.Provider value={{ isDark, handlerChange }}>{children}</ThemeContext.Provider>
}
...
//   const systemTheme = theme ? 'dark' : 'light'
//   return systemTheme
// }
